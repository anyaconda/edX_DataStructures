...
class Tree:
    ...
    def computeHeight(self, node):
        ...
        while self.queue:
            kids = list(np.where(self.parent == node)[0])
            if kids:
                self.queue.enqueue(kids)
                self.queue.dequeue()
                #self.height += 1
                print('size', self.queue.size())
                for i in range(self.queue.size()):
                    #print(i)
                    self.computeHeight(self.queue.dequeue())
            else:
                return
            ...
            print ('empty? ', self.queue.isEmpty())


    def computeHeight2(self, node):
